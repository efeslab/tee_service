...
class SGXCryptoWrapperSGXError( Exception ):
	def __init__( self,  errCode ):
		msg = "SGX Error 0x%x" % errCode
		Exception.__init__( self, msg )
		self.errorCode = errCode

class SGXCryptoWrapperBadParameterError( Exception ):
    def __init__( self,  msg ):
        errMsg = "Bad parameter given: " + msg
...
class SGXCryptoWrapper():
    ...
    def SignECDSA( self, dataToSign, signingKey ):
        ...
        dataToSign_c = ( c_uint8 * len( dataToSign )    ).from_buffer( dataToSign )
        signingKey_c = ( c_uint8 * len( signingKey )    ).from_buffer( signingKey )
    ...
    def Rijndael128GCM_Encrypt( self, key, iv, dataToEncrypt, additionalAuthenticatedData ):
        self.log.debug( "Rijndael128GCM_Encrypt" )

        if( len( iv ) != SGX_AESGCM_IV_SIZE ):
            raise SGXCryptoWrapperBadParameterError( "len( IV ) MUST BE %d, but instead len( IV ) == %d" % ( SGX_AESGCM_IV_SIZE, len( iv ) ) )

        if( len( key ) != EC_KEY_128_BIT_SIZE):
            raise SGXCryptoWrapperBadParameterError( "len( key ) MUST BE %d, but instead len( key ) == %d" % ( EC_KEY_128_BIT_SIZE, len( key ) ) )

        key_c           = ( c_uint8 * len( key )            ).from_buffer( key )
        iv_c            = ( c_uint8 * len( iv )             ).from_buffer( iv )
        dataToEncrypt_c = ( c_uint8 * len( dataToEncrypt )  ).from_buffer( dataToEncrypt )
        cipherText      = ( c_uint8 * len( dataToEncrypt )  )() #same size is source plain text
        mac             = ( c_uint8 * SGX_AESGCM_MAC_SIZE   )()

        dataToEncryptSize = c_uint32( len( dataToEncrypt )  )
        ivSize            = c_uint32( len( iv ) )
        
        
        if additionalAuthenticatedData != None:
            aad_c   = ( c_uint8 * len( additionalAuthenticatedData ) ).from_buffer( additionalAuthenticatedData )
            aadSize = c_uint32( len( additionalAuthenticatedData ) )
        else:
            aad_c   = None #translates to NULL pointer
            aadSize = c_uint32( 0 )

        result = self.implementation.sgx_rijndael128GCM_encrypt( key_c,
                                                                 dataToEncrypt_c,
                                                                 dataToEncryptSize,
                                                                 cipherText,
                                                                 iv_c,
        ...
        return bytearray( cipherText ), bytearray( mac )

    def Rijndael128GCM_Decrypt( self, key, iv, dataToDecrypt, additionalAuthenticatedData, mac ):
        self.log.debug( "Rijndael128GCM_Decrypt" )
        ...
        if( len( mac ) != SGX_AESGCM_MAC_SIZE):
            raise SGXCryptoWrapperBadParameterError( "len( mac ) MUST BE %d, but instead len( mac ) == %d" % ( SGX_AESGCM_MAC_SIZE, len( mac ) ) )

        key_c           = ( c_uint8 * len( key )            ).from_buffer( key )
        iv_c            = ( c_uint8 * len( iv )             ).from_buffer( iv )
        dataToDecrypt_c = ( c_uint8 * len( dataToDecrypt )  ).from_buffer( dataToDecrypt )
        plainText       = ( c_uint8 * len( dataToDecrypt )  )() #same size is source ciphertext
        mac_c           = ( c_uint8 * len( mac )            ).from_buffer( mac )

        dataToDecryptSize = c_uint32( len( dataToDecrypt )  )
        ivSize            = c_uint32( len( iv ) )
        ...
        result = self.implementation.sgx_rijndael128GCM_decrypt( key_c,
                                                                 dataToDecrypt_c,
                                                                 dataToDecryptSize,
                                                                 plainText,
                                                                 iv_c,
    ...
    def VerifyResult( self, result ):
        if result == SGXStatus.SGX_SUCCESS:
            ...
            self.log.error( "SGX Error 0x%x" % result )
            raise SGXCryptoWrapperSGXError( result )
...
class TestSGXCryptoWrapper(unittest.TestCase):
    ...
    def test_signECDSA( self ):
        ...
        # # Speed test:
        # self.cryptoWrapper.log.setLevel(0)
        ...
        #   ( totalTime, NUM_OPERATIONS , NUM_OPERATIONS / totalTime ) )
        # print( "#" * 50 )

    def test_Rijndael128_CMAC( self ):
        macKey    = os.urandom( SGX_AESGCM_MAC_SIZE )
        dataToMAC = os.urandom( 1000 )

        mac1 = self.cryptoWrapper.Rijndael128_CMAC( dataToMAC, macKey )
        mac2 = self.cryptoWrapper.Rijndael128_CMAC( dataToMAC, macKey )

        # print( "mac1 = %s" % bytearray( mac1 ) )
        # print( "mac2 = %s" % bytearray( mac2 ) )

    def test_Rijndael128GCM_Encrypt_Decrypt_no_AAD( self ):
        key         = bytearray( os.urandom( EC_KEY_128_BIT_SIZE ) )
        iv          = bytearray( os.urandom( SGX_AESGCM_IV_SIZE ) )
        plaintext   = bytearray( os.urandom( 2000 ) )

        NO_AAD          = None
        cipherText, mac = self.cryptoWrapper.Rijndael128GCM_Encrypt( key, iv, plaintext, NO_AAD )
        decryptedText   = self.cryptoWrapper.Rijndael128GCM_Decrypt( key, iv, cipherText, NO_AAD, mac )

        self.assertTrue( plaintext == decryptedText )

        #Speed test:
        # self.cryptoWrapper.log.setLevel(0)
        # plaintext       = bytearray( os.urandom( 1024 ) )
        # NO_AAD          = None
        # NUM_OPERATIONS  = 1000000
        ...
        # #print performance:
        # print( "#" * 50 )
        # print( "AES_GCM Encrypt 1 KB: Total time: %f, num operations: %d, num operations per second: %f" % \
        #   ( totalTime, NUM_OPERATIONS , NUM_OPERATIONS / totalTime ) )
        # print( "#" * 50 )
